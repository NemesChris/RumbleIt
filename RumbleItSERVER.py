...
def sample_first_joystick():
    ...
    while True:
        ...
        if len(str(data)) > 0:
            ...
            if (float(slipFL) + float(slipRL))/2 >= (float(suspFL) + float(suspRL))/2:
                left_speed = (float(slipFL) + float(slipRL)/2)
            else:
                left_speed = (float(suspFL) + float(suspRL))/2

            if (float(slipFR) + float(slipRR))/2 >= (float(suspFR) + float(suspRR))/2:
                right_speed = (float(slipFR) + float(slipRR))/2
            else:
                right_speed = (float(suspFR) + float(suspRR))/2
